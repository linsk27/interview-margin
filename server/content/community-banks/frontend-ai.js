...
export const frontendAiInterviewBank = {
  ...
  category: '真实面经',
  description: '精选公开前端与 AI 应用面经中的高频技术题，覆盖流式交互、生成式 UI、RAG、Agent、模型网关、安全与评估；答案强调场景、选型、失败路径、指标和兜底。',
  ...
  sections: [
    ...
    },
    {
      ...
      questions: [
        {
          title: 'RAG 从文档入库到带引用回答的完整链路如何设计？',
          summary: '完整 RAG 不是“向量库加 Prompt”，而是可追溯的采集、清洗、分块、索引、检索、排序、上下文组装、生成、引用与评估链路。',
          mechanism: '入库侧保留 documentId、版本、权限、来源位置和校验和，清洗正文后按语义与版面分块，为 chunk 建稠密向量并保留关键词索引。查询侧先做权限与意图处理，再并行稀疏、向量召回，融合去重后用 reranker 精排；上下文组装控制 token 预算、相邻片段和来源多样性。模型只基于编号证据回答并输出引用 id，服务端验证引用确实存在后映射到原文位置。全链路记录 query、候选、分数、最终上下文、模型版本和反馈，用检索与回答两层指标定位失败。',
          example: '员工问“报销打车的上限是多少”，系统先按部门和制度生效日期过滤，BM25 找到“交通费”，向量召回找到“网约车”，重排后拼入同一制度的两段证据。回答引用制度版本与页码；若没有达到证据阈值，则明确说未找到并给出相关制度入口，而不猜金额。',
          followUps: [
            { question: '检索到了正确文档却答错，先查哪里？', answer: '先核对正确 chunk 是否进入最终上下文、是否被截断，再看提示约束和引用映射；这属于生成或组装问题，不能只调召回 TopK。' },
        ...
        {
          title: '固定、语义、重叠与父子分块如何用数据选择？',
          summary: '分块要同时满足“能被召回”和“足够回答”；应按文档结构建立候选策略，再用真实问题评测命中率、上下文完整度和成本。',
          mechanism: '固定 token 分块简单稳定，适合结构弱的长文本；标题、段落或代码函数等语义分块能保留局部完整性；重叠可缓解边界断裂，但会增加索引、重复召回和上下文浪费；父子分块用小子块做精准召回，再取较大父块补足语境。选型时用同一批带证据标注的问题，对不同 chunk size、overlap 与层级策略测 evidence recall、MRR、重复率、最终答案正确率、平均上下文 token 和 P95 时延。不同文档类型可以使用不同解析器，不必追求全库一个尺寸。',
          example: 'API 文档以“端点—参数—示例”作父块，参数段作子块；用户问错误码时由 180-token 子块命中，再带回 700-token 端点父块。合同则按条款分块并保留标题路径。评测发现 20% 重叠只提升 0.6% 命中却增加 17% token，最终改为边界处相邻块扩展。',
          followUps: [
            { question: 'chunk 越小是否召回越准？', answer: '小块可能更聚焦，但会丢失定义、条件和例外，导致生成阶段无法回答；要看证据覆盖和最终答案，而不是只看相似度。' },
            { question: '父块应该全部送进模型吗？', answer: '不一定。可按子块得分、问题类型和 token 预算选择父块或相邻窗口，并去掉重复内容，避免一个命中吞掉全部上下文。' },
          ],
          pitfalls: [
        ...
        {
          title: 'BM25 与向量召回怎样融合，权重为什么不能拍脑袋？',
          summary: 'BM25 擅长精确术语、编号与罕见词，向量检索擅长同义表达；融合应基于排名归一化和标注查询调参，而非直接相加异构分数。',
          mechanism: '两路分数分布不可直接比较：BM25 无上界，不同向量距离含义也不同。常见做法是各自取候选后用 Reciprocal Rank Fusion 按名次合并，或先校准分数再做加权；随后去重并进入 cross-encoder rerank。权重按查询类型也可动态调整，例如带错误码、产品型号或人名时提高稀疏权重，口语化意图提高向量权重。离线用带相关性等级的数据测 Recall@K、nDCG、MRR，线上看回答成功率、零结果率和时延；必须保留单路基线，确认融合增益来自互补而不是 TopK 变大。',
          example: '查询“ERR_CONNECTION_CLOSED 怎么排查”时，BM25 精确命中错误码，向量路补充“连接被代理关闭”的排障文档。系统分别取 40 条，以 RRF 合并到 60 条再重排 10 条；权重通过 500 个真实查询网格搜索，而不是把 BM25 分数乘 0.7、余弦分数乘 0.3 后直接相加。',
          followUps: [
            { question: 'RRF 为什么常比直接加分稳？', answer: '它只依赖各检索器内部排名，避免异构分数量纲和分布漂移；但它仍有超参数，且无法替代有监督重排。' },
            { question: '如何识别查询需要更高关键词权重？', answer: '可用规则识别编号、专有名词和引号短语，也可训练轻量路由器；路由决策要记录并在分桶评测中验证。' },
          ],
          pitfalls: [
        ...
        {
          title: '粗召回与 Rerank 如何分工，TopK 怎样确定？',
          summary: '粗召回用低成本保证正确证据进入候选集，Rerank 用更强的查询—文档交互提升前排精度；两阶段 TopK 由召回曲线、预算和时延共同决定。',
          mechanism: '向量或混合检索负责从百万级语料快速取几十到几百条，其目标是高 Recall@K；reranker 对 query 与每个候选成对打分，捕捉否定、条件和词序，目标是让真正相关片段进入有限上下文。先画粗召回 K 从 10 到 200 的证据召回曲线，找到收益拐点，再测试 rerank 输入数与输出数对 nDCG、答案正确率、P95 时延和成本的影响。输出还要做文档多样性、权限复核与邻块合并。高置信简单查询可跳过重排，超时则退回融合排名，并显式记录降级。',
          example: '企业问答中粗召回 K=80 时证据召回 94%，增至 160 仅升 0.4% 却令重排时延翻倍，因此保留 80；rerank 取前 8，再按 token 预算装入 5 段。重排服务超过 300ms 时使用 RRF 前 5 段，同时给 trace 标记 rerank_timeout 便于复盘。',
          followUps: [
            { question: 'Rerank 是否能救回粗召回漏掉的文档？', answer: '不能，重排只在候选集合内排序；正确证据未进入候选时应修分块、索引、查询改写或粗召回策略。' },
            { question: '为什么最终不一定取分数最高的连续几块？', answer: '它们可能来自同一文档并高度重复，需要兼顾来源多样性、覆盖不同子问题和 token 预算，避免上下文单一。' },
          ],
          pitfalls: [
        ...
        {
          title: '口语化 Query 召回失败时如何改写而不改变意图？',
          summary: 'Query 改写要补全省略、同义词和上下文实体，但必须保留原查询、可验证约束与回退路径，防止“改得更像文档却答非所问”。',
          mechanism: '先判断失败类型：错别字、指代、省略、领域别名或复合问题，再选择规则规范化、词典扩展、对话补全或模型生成多查询。改写提示只允许补充已知上下文，不得发明用户身份、时间或业务条件；原 query 与若干改写并行检索，结果融合而不是完全替换。对于多意图先拆子查询，再在答案层合并。离线标注 intent preservation、evidence recall 和误扩展率，线上保留改写文本用于可观测与申诉；低置信、高风险或含明确专有词时优先使用原查询。',
          example: '用户在报销对话里问“这个周末也行吗”，系统从上一轮只补成“周末发生的网约车费用是否可报销”，同时保留原句并行检索。若模型改写出“加班打车”但上下文没有加班条件，该版本因新增约束被拒绝；无可靠证据时反问用户场景。',
          followUps: [
            { question: '为什么多查询可能降低效果？', answer: '错误改写会引入噪声并挤占候选名额，多个近义查询也会重复召回；需要限制数量、融合去重并评测意图保持率。' },
            { question: '如何验证改写没有改变用户意图？', answer: '检查实体、时间、否定和权限条件是否守恒，并用成对标注或规则约束；不确定时同时检索原句或向用户澄清。' },
          ],
          pitfalls: [
    ...
    },
    {
      ...
      questions: [
        ...
        },
        {
          ...
          followUps: [
            { question: '更高维一定更好吗？', answer: '不一定，高维增加存储与计算，模型训练质量和领域匹配更关键；必须在同一语料与检索配置下用真实任务比较。' },
        ...
        },
        {
          ...
          followUps: [
            { question: 'Precision@K 下降就一定要减少 K 吗？', answer: '不一定，若正确证据仍在前排且噪声不进入上下文，最终效果可能不变；应联合看排序、上下文和答案指标。' },
      ...
      questions: [
        ...
        },
        {
          ...
          followUps: [
            ...
            { question: '什么时候应把逻辑下沉 MCP 工具？', answer: '稳定、可复用且必须在可信边界执行的校验或原子业务操作应下沉；面向任务的顺序、取舍和交互留在 Skill 或 Workflow。' },
          ],
          pitfalls: [
      ...
      questions: [
        ...
        },
        {
          ...
          followUps: [
            ...
            { question: '熔断和限流有什么区别？', answer: '限流控制允许进入的负载，熔断根据依赖健康主动停止调用；两者配合但触发依据和目的不同。' },
          ],
          pitfalls: [
      ...
      questions: [
        {
          ...
          followUps: [
            { question: '用户点赞率能作为核心价值指标吗？', answer: '通常只能作信号，容易受界面和新鲜感影响；应与真实任务完成、采纳、返工和业务结果结合。' },
        ...
        },
        {
          ...
          followUps: [
            ...
            { question: '人工 Review 应重点看什么？', answer: '先看需求和信任边界，再看失败路径、数据迁移、回滚和测试证据；代码风格通常不是最高风险。' },
          ],
          pitfalls: [
        ...
        },
        {
          ...
          followUps: [
            { question: 'LLM 作为评委是否可信？', answer: '可用于规模化信号，但要有清晰 rubric、盲测顺序、多人或多次一致性检查，并定期与人工标注校准偏差。' },
