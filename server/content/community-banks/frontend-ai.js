  ...
  sections: [
    {
      ...
      questions: [
        ...
        },
        {
          ...
          summary: '服务器发来的一条消息，可能被网络拆成几小段，也可能和下一条消息一起到达。前端要先连续解码文字，再把残片拼起来；只有看到一条完整事件的结束标记后才解析。',
          mechanism: '把网络响应想成一卷被随手剪开的纸带。服务端即使一次写入 `data: {"text":"你好"}\\n\\n`，浏览器第一次 `read()` 也可能只拿到 `data: {"text":"你`，下一次才拿到剩余内容。第一层用同一个 `TextDecoderStream` 连续解码，避免一个中文字的 UTF-8 字节被拆开后变成乱码；第二层把文字追加到 `buffer`，以 SSE 的空行分隔符找出完整事件，最后一个不完整片段继续留在 `buffer`；第三层才把事件中的 `data:` 合并并执行 `JSON.parse`。用户点“停止”时，`AbortController` 的同一个 `signal` 要交给 `fetch` 和读取逻辑，让网络、解析和后续状态更新一起停下，而不只是隐藏加载动画。',
          example: '例如服务端连续发送“你好”和“世界”两条 SSE 事件，网络可能把它们切成三次返回：第一段停在“你”的字节中间，第二段包含上一条结尾和下一条开头，第三段才出现空行。正确代码无论怎样切，最后都只派发两条完整事件。用户主动停止时保留已经展示的文字并显示“已停止”；网络断开或 JSON 格式错误则显示“连接失败/响应格式异常”和重试入口，不能把三种状态混成同一个红色报错。',
          followUps: [
      ...
      questions: [
        {
          ...
          summary: 'MySQL 保存“这条业务数据现在是什么”的权威记录；Milvus 保存文本换算出的语义坐标，负责从大量资料中快速找出意思相近的候选。前者擅长事务和关联查询，后者擅长近似搜索，所以要用稳定 ID、版本和删除状态把两边对齐，不能互相替代。',
        ...
        },
        {
          ...
          summary: 'Embedding 模型可以理解为“把文字放进语义坐标系的规则”。模型或向量维度一变，新旧坐标就不能可靠混查，因此应新建索引、逐步回填并让新旧结果并行对照；确认质量、时延和成本达标后再切换，同时保留回滚入口。',
        ...
        },
        {
          ...
          summary: '召回率提高表示“正确资料更容易被找进候选”，精准率下降表示“候选里无关资料也更多了”。应先按问题类型找出噪声从哪一条检索通道进入，再调整过滤、合并排序和二次重排；只把候选数 TopK 调小，可能把正确资料也一起删掉。',
        ...
        },
        {
          ...
          summary: '上下文就是本轮交给模型参考的资料片段。太短会漏掉回答前提，太长会增加费用并让无关信息干扰判断。应先确保关键证据覆盖问题，再去重、补齐必要相邻段，并在证据不足时拒答；Token 预算只是容量上限，不是越塞满越好。',
        ...
        },
        {
          ...
          summary: '系统找资料之前就要按用户权限过滤，取回原文时再检查一次，避免“模型看到了但页面不展示”的越权。引用必须指向用户能访问的具体版本；文档更新则生成新版本索引，并用可重复执行的任务处理失败和删除，保证答案与出处一致。',
      ...
      questions: [
        {
          ...
          summary: '普通大模型（LLM）只负责一次生成或判断；确定性 Workflow 像写好的流程图，步骤由程序决定；Agent 则让模型在受限工具中动态选择下一步。流程越固定、操作风险越高，越应使用 Workflow；只有路径确实无法预先确定时才增加 Agent 自主性。',
        ...
        },
        {
          ...
          summary: '传统 API 规定两个软件怎样交换请求和响应；Function Calling 只让模型按固定结构提出“调用哪个函数、传什么参数”，真正执行仍由应用决定；MCP 再往外统一 AI 客户端如何发现和连接不同服务提供的工具、资料与提示。三者分别解决通信、模型表达调用意图和外部能力接入。',
        ...
        },
        {
          ...
          summary: 'Skill 是一份可复用的做事说明书，记录步骤、约束和验收方法；MCP 是连接外部工具与资料的标准接口。Skill 回答“这类任务应该怎样完成”，MCP 回答“当前能连接并调用哪些能力”，所以常常配合使用，却不能互相替代。',
        ...
        },
        {
          ...
          summary: 'Sub-Agent 就是只负责一个明确子任务的独立执行者。只有工作可以并行、需要不同专业背景或能独立验收时才值得拆分；同时必须隔离它能看到的上下文、工具、权限、预算和文件范围，最后由主流程检查并合并结果。',
        ...
        },
        {
          ...
          summary: '工具链每完成一步都要保存状态，并记录这一步是否已经写库、发消息或产生其他外部影响。查询类步骤通常可以重试；可能重复扣款、发信的步骤要用唯一幂等键阻止重复，或准备反向补偿。无法确认当前状态时，应停止自动执行并把已完成步骤和证据交给人工。',
      ...
      questions: [
        {
          ...
          summary: '短期记忆只帮助完成当前对话，长期记忆保存用户明确确认、以后仍有用的偏好，业务数据库才保存订单、权限等权威事实。模型本轮看到的上下文只是临时拼出的材料，可能过期或被裁剪，不能反过来当数据库使用。',
        ...
        },
        {
          ...
          summary: '当对话材料接近模型容量，或已经明显拖慢、变贵时才压缩。安全规则、当前目标、尚未完成的承诺、关键对象和工具结果必须保留；旧闲聊可以摘要。压缩后要用固定任务重新执行，确认没有丢掉会改变答案或操作结果的条件。',
        ...
        },
        {
          ...
          summary: '旧记忆和工具日志都可能过期、属于别的任务或记录失败结果，因此每条都要保存来源、时间、适用范围和可信程度。再次使用前先过滤过期内容、处理相互冲突的记录，只把当前任务真正需要的部分交给模型。',
        ...
        },
        {
          ...
          summary: '先淘汰不满足安全、上下文长度或工具能力的模型，再在合格模型中比较答案质量、费用和高峰时延。P95 时延表示 95% 请求都能在这个时间内完成，比平均值更能反映慢请求。路由规则必须能用同一评测集复现，并在候选模型异常时回退。',
        ...
        },
        {
          ...
          summary: '先给整次请求规定一个最晚结束时间，而不是让每一层各等一遍；只对不会产生重复副作用的请求做有限重试。依赖持续超时或报错时，熔断器会暂时停止继续调用，避免故障拖垮更多请求；切到备用模型前还要确认输出格式、工具能力、质量和预算都兼容。',
