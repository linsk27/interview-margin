...
const PUBLIC_SAFE_SHORTS = new Map([
  ...
  ['5.4', 'RRF 只看名次，不直接相加不可比的原始分数。因为 BM25 分数和向量相似度的含义、范围都不同，直接相加会让某一路仅因数值更大而压过另一条检索链；换成“第几名”后再按名次倒数累加，才是在比较同一把尺子。'],
  ...
  ['7.3', [
    '三次握手是为了让双方确认双向收发能力并同步初始序列号；四次挥手来自 TCP 两个发送方向要分别关闭；TIME_WAIT 则用于补发最后的 ACK，并等旧连接报文过期，避免污染随后复用相同四元组的新连接。',
    '',
    '第一次，客户端发送 SYN 和初始序列号 x。第二次，服务端返回 SYN 加 ACK，自己的初始序列号是 y，确认号是 x+1。第三次，客户端再返回 ACK，确认号是 y+1，双方进入已连接状态。',
    '',
    '```text',
    'Client                         Server',
    '  | ---- SYN, seq=x ----------> |',
  ...
  ['8.2', '若链表按低位在前存储，就同步遍历两个链表：每一位计算 `sum = x + y + carry`，写入 `sum % 10`，再把 `Math.floor(sum / 10)` 作为进位留给下一位。循环必须覆盖两个链表和最后一次进位，时间为 `O(max(m,n))`；若题目使用高位在前，则先反转或改用栈。'],
  ...
  ['9.6', '隔离级别决定一个事务能看见其他并发事务的哪些变化：Read Uncommitted 可能脏读，Read Committed 仍可能不可重复读，Repeatable Read 让普通快照读保持一致视图，Serializable 约束最强但并发代价最高。具体数据库对幻读、当前读和锁的实现不同，不能只背一张标准表。'],
...
const PUBLIC_SAFE_MECHANISMS = new Map([
  ...
  ['4.1', [
    '- 因为默认 ATT MTU 常为 23 byte，而写入或通知还要占用 3 byte 左右的协议头，所以应用最常见的可用载荷是 20 byte；协商后的 MTU 和具体操作不同，这个数也会变化。',
  ...
  ['4.2', `配置流程适合状态机，是因为状态机把合法状态、可接受事件和迁移条件集中写成一张可验证的“交通图”，能直接拒绝跳步和“同时成功又失败”这类非法组合。它把流程从零散的 if/else 提升为一个可验证的迁移模型：
...
非法事件应被拒绝或记录，不能悄悄跳步。页面刷新后只恢复可序列化状态，外设连接和未完成请求必须重新确认，这是流程状态与外部世界的边界。`],
  ['4.4', `Promise.allSettled 适合批量上传，是因为单个任务失败不会让结果收集提前中断；它会等每个输入都结束，再让页面逐项显示成功、失败和可重试状态。它会为每个输入建立独立的成功和失败处理，直到所有输入都已终止才解决聚合 Promise：
...
因此完整的批量上传器还需要并发池、AbortSignal、进度模型、幂等键和失败重试策略。如果任一文件失败都不允许继续，则应使用更强的整体事务或补偿机制，而不是只换一个 Promise API。`],
  ['5.3', `关键词检索和向量检索要结合，是因为它们会漏掉不同类型的问题：关键词检索能稳稳命中错误码、编号和原句，却不认识很多同义改写；向量检索能理解相近意思，却可能忽略罕见专有名词。两路先各自召回，再用 RRF 或经过校准的分数融合，才能互相补漏；最后仍要用同一批真实问题做对照评测，不能把“混合”当成必然提升。`],
  ['5.4', [
    '因为 BM25 的 12.4 与余弦相似度的 0.82 不能直接比较，但“两路都排前五”是可比较的信号，所以 RRF 把各路原始分数换成名次后再融合：',
    '',
    '```text',
  ...
  ['5.5', `MMR 能减少重复上下文，是因为每次选下一段资料时，它不只看“和问题多相关”，还会扣掉“和已经选中的资料多重复”。它不是另一种向量检索，而是对已召回候选集做迭代选择。每轮都给未选文档计算：
...
算法选出当前最高分项后把它加入已选集，再重复到 Top-K 满足。两类相似度必须使用可比较的尺度，候选集过小时 MMR 也无法找回未被召回的证据。评估时应同时检查答案覆盖率和重复率，防止为了“多样”选入不相关片段。`],
  ['5.6', `RAG 评测必须拆开检索、生成和端到端三层，因为最终答错既可能是资料没找回来，也可能是排序错误，或模型拿到正确证据后仍生成错；只看一个总分无法定位该改哪一层。具体做法是：
...
每次实验必须锁定文档、切块、Embedding、索引、Prompt 和模型版本，再按“没召回”“召回错”“有证据却生成错”分桶复盘。`],
  ['5.7', `Embedding 必须做版本管理，因为模型参数、训练目标或归一化方式一变，就相当于换了一套坐标系；即使向量长度相同，新查询向量也不能和旧文档向量可靠比较。迁移时要并行建立新索引，验证完整性与检索效果后原子切换别名，并保留旧索引用于回滚。`],
...
因果掩码会把未来位置的分数设为不可选，保证 Decoder 只利用已有 token。多头注意力在不同投影子空间并行计算后再合并；某个头可以学到特定模式，但不应假设每个头都对应固定的人类语义。`],
  ['5.16', `很多生成式大模型选择 Decoder-only，主要因为提示和答案可以放进同一条序列，用同一个“预测下一个 Token”目标训练和推理；模型结构、数据接口和任务形式更统一，比较容易扩展到续写、对话和上下文学习。Encoder 并没有消失：它仍常用于分类和向量表示，Encoder–Decoder 仍适合输入到输出的转换任务。

- Decoder-only 用 Causal Mask 保证位置 \`t\` 只能看见 \`≤t\` 的 Token。
- KV Cache 保存历史层的 Key/Value，减少逐 Token 生成时的重复计算，但不会消除缓存占用。
- “主流”不等于“所有任务都更好”，架构仍取决于任务目标、延迟、吞吐和训练方式。`],
  ['6.4', `- 因为网络按任意字节位置拆包，而一个中文字符通常由多个 UTF-8 字节组成，所以某个 chunk 可能只拿到“半个字符”，单独解码就会产生替换字符或乱码。
...
- 测试时应故意切开中文字节、\`\\r\\n\`、\`\\n\\n\` 和 JSON 引号，不能只测“恰好一个 chunk 一个事件”的理想情况。`],
  ['6.5', `- 因为应用、压缩器、反向代理或 CDN 都可能先把小块放进缓冲区，直到攒够大小或响应结束才向下游刷新，所以后端已经 \`yield\` 并不代表浏览器立即收到。
...
预检成功不代表业务请求有权限，预检失败也不代表服务器收不到所有跨源请求。鉴权、CSRF 防护和输入校验必须独立完成。`],
  ['7.3', `因为 TCP 要先证明双向链路可用、关闭时又要分别结束两个发送方向，还要防止最终 ACK 丢失和旧报文污染新连接，所以分别需要三次握手、通常四次挥手和 TIME_WAIT。
...
这些任务最终仍在系统线程上执行。选型应同时看故障隔离、共享状态、CPU/I/O 比例、运行时调度、取消传递和调试成本，不能只比“谁更轻”。`],
  ['7.6', `因为每个进程都需要连续、隔离的地址视图，而物理内存实际会分散、共享并按需装入，所以虚拟内存用页表把程序看到的虚拟页映射到物理页框：
