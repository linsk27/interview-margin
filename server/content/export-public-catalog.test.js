...
describe('public catalog exporter', () => {
  it('exports exactly the public, active seeded catalog', () => {
    ...
    expect(catalog.banks).toHaveLength(9)
    expect(questions).toHaveLength(501)
    expect(new Set(questions.map((question) => question.id)).size).toBe(501)
  ...
  it('writes byte-for-byte deterministic JSON and creates the destination directory', () => {
    ...
    expect(firstResult).toMatchObject({ banks: 9, questions: 501, outputPath: first })
