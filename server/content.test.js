...
describe('question catalog seed quality', () => {
  ...
  it('contains exactly ten banks and 573 unique questions', () => {
    expect(db.prepare('SELECT COUNT(*) count FROM question_banks').get().count).toBe(10)
    expect(db.prepare('SELECT COUNT(*) count FROM questions').get().count).toBe(573)
    expect(db.prepare('SELECT COUNT(DISTINCT id) count FROM questions').get().count).toBe(573)
