...
const PAGE_TURN_FALLBACK_MS = PAGE_TURN_MS + 320
...
function sanitizePageClone(root: HTMLElement) {
...
function copyNestedScrollPositions(source: HTMLElement, clone: HTMLElement) {
  const sourceScrollers = source.querySelectorAll<HTMLElement>('.code-block pre, .table-wrap')
  const cloneScrollers = clone.querySelectorAll<HTMLElement>('.code-block pre, .table-wrap')
  sourceScrollers.forEach((scroller, index) => {
    const clonedScroller = cloneScrollers[index]
    if (!clonedScroller) return
    clonedScroller.scrollTop = scroller.scrollTop
    clonedScroller.scrollLeft = scroller.scrollLeft
  })
}
...
function createPageCopy(
  ...
  page.className = `reader__flip-page reader__flip-page--${side}`
  page.setAttribute('aria-hidden', 'true')
  page.setAttribute('inert', '')

  const clone = flow.cloneNode(true) as HTMLDivElement
  clone.classList.add('reader__flip-flow')
  positionSpreadFlow(clone, spreadIndex, geometry)
  copyNestedScrollPositions(flow, clone)
...
function createSpreadFlip(
  ...
  staticHalf.append(createPageCopy(flow, currentIndex, geometry, staticSide))
  ...
  front.append(createPageCopy(flow, currentIndex, geometry, movingOldSide))

  const back = document.createElement('div')
  back.className = 'reader__flip-face reader__flip-face--back'
  back.append(createPageCopy(flow, nextIndex, geometry, movingNewSide))

  const paper = document.createElement('div')
  paper.className = 'reader__flip-paper'
...
export function Reader({
  ...
  const turnSpread = (direction: TurnDirection) => {
    ...
    try {
      ...
      const flow = flowRef.current
      if (!layer || !flow) throw new Error('Reader spread is not mounted')

      const animationTarget = createSpreadFlip(
        layer,
        flow,
        currentIndex,
        nextIndex,
      ...
      const handleAnimationEnd = (event: AnimationEvent) => {
        if (event.target !== animationTarget || !event.animationName.startsWith('reader-page-turn-dom-')) return
      ...
      applySpread(nextIndex)
      console.warn('Page-turn layer failed; using an immediate page change.', error)
      finishSpreadTurn(sequence)
    }
  }
