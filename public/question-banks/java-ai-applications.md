...
2. **LLM-as-judge 可以完全替代人工吗？**
   ...
   不能。评审模型也有偏差和漂移，需要人工基准校准，并对高风险或分歧样本复核。
...
1. **目标证据在候选中却没进最终上下文，检查什么？**
   ...
   检查阈值、ACL、融合权重、去重、重排和 Token 预算，尤其注意重复相邻 chunk 是否挤占前 k。
...
1. **Agent 与普通聊天机器人有什么区别？**
   ...
   普通聊天多是一次或少量生成；Agent 具有显式目标、工具、状态和多步反馈循环，应用还必须治理动作与终止。
...
2. **什么请求不应该自动降级模型？**
   ...
   涉及固定模型评测、合规地域、强结构化能力或高风险判断时不能静默切换，应失败或要求确认。
...
1. **SSE 与 WebSocket 怎样选？**
   ...
   主要是服务端向浏览器持续推送文本时 SSE 更简单；需要高频双向消息时再考虑 WebSocket，二者都必须处理断线和取消。
