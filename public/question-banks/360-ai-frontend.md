...
## Q1：如何组织一段 90 秒的技术岗位自我介绍？

**短回答：**

一段合格的 90 秒技术岗位自我介绍只回答三件事：当前定位、与岗位直接相关的两条可验证证据、能力边界与求职动机。可按“15 秒定位 + 55 秒项目证据 + 20 秒边界和动机”组织，姓名、学校和项目名称只作为必要背景，不应挤占能力证据。

**原理：**

面试开场的注意力预算很短，因此需要先给结论，再给证据。定位句让面试官建立候选人模型，项目证据证明能力不是标签，边界与动机则降低夸大风险并解释岗位选择。每一段都应能被后续追问验证，无法展开的技能不应放进主线。

**代码 / 场景：**

先选定目标岗位，再录制 90 秒和 45 秒两个版本。前 15 秒只说当前定位，中间用两个“问题—行动—证据”片段证明匹配，结尾主动说明一项能力边界与求职动机。回听时删除学校沿革、项目清单和无法验证的形容词，确保被打断后仍能留下核心证据。

**递进追问：**

1. **如果只剩 45 秒，哪些内容必须保留？**
...
AI 应用前端位于模型能力和真实产品之间，除了普通页面状态，还要处理上下文、流式输出、引用、取消、工具过程、失败恢复和不可信内容。适合这个方向的证据应来自可运行的交互链路，而不是只说“AI 是趋势”。
...
- 不要把 AI 前端简化为“调一个大模型接口”，这会忽略状态一致性与安全边界。
...
## Q3：为什么选择 360 和 PC 安全与办公事业部？

**短回答：**

回答公司动机时要建立“公开产品问题—岗位职责—可迁移能力”三层对应。对 360 可以基于公开的 AI 浏览器、企业知识库、安全与办公场景说明理解，但不能猜测具体小组，也不能把未体验的产品说成深度使用。

**原理：**

回答“为什么选择 360 和 PC 安全与办公事业部？”时，先把岗位需要的能力拆成可验证要求，再用一到两个真实经历建立对应关系。表达顺序是结论、证据、个人贡献、边界与下一步；不要把团队成果全部归到个人，也不要用尚未测量的数据支撑结论。

**代码 / 场景：**

回答前先核对岗位 JD 与 360 现有 AI 浏览器、企业知识库能力，把动机落到三个可验证交集：PC 场景中的长文本与网页上下文、实时 AI 交互、前后端协作。准备一个入职后 30 天计划，包含熟悉产品链路、补 React 工程习惯和完成一个小型 Go 接口。

**递进追问：**

1. **为什么不是泛泛地选择任何一家做 AI 的公司？**
...
## Q4：如何用两分钟讲清 AI 知识库项目的用户、问题、方案和贡献？

**短回答：**

两分钟项目介绍应先说明用户和痛点，再讲输入输出与核心闭环，随后明确个人负责范围、一个关键取舍、可演示证据和当前边界。技术栈只用于解释方案，不能代替产品问题和验证结果。

**原理：**

项目介绍本质上是在有限时间内建立一条可验证的因果链：某类用户遇到具体问题，系统接收什么输入、经过哪些关键状态、产出什么结果；候选人负责其中哪一段、为什么选择该方案、如何验证，以及尚未解决什么。缺少任一环，介绍都容易退化成技术名词或功能清单。

**代码 / 场景：**

用一张四格卡片演练 AI 知识工作台：用户是需要反复整理资料的人，问题是上下文分散且回答不可追溯，方案是文档切块、检索、上下文包与 SSE，个人贡献是前后端链路和交互状态。现场演示一条查询，并在两分钟结束前主动说出目前尚未完成的评测和生产化能力。

**递进追问：**

1. **AI 知识工作台 相比直接把文件上传给聊天模型多解决了什么？**
...
- 不要从技术栈开始讲两分钟，面试官首先需要理解用户问题和产品闭环。
...
回答“为什么使用 RAG，而不是全文 Prompt、普通搜索或微调？”时，先把岗位需要的能力拆成可验证要求，再用一到两个真实经历建立对应关系。表达顺序是结论、证据、个人贡献、边界与下一步；不要把团队成果全部归到个人，也不要用尚未测量的数据支撑结论。
...
## Q6：项目周期较短时，如何说明复用、增量与完成度？

**短回答：**

短周期交付必须拆清复用部分与新增部分，并用提交记录、测试、演示和未完成清单证明范围。可信的说法是“基于已有底座完成若干增量模块”，而不是把复用、依赖库和 AI 辅助全部包装成从零开发。

**原理：**

回答“项目周期较短时，如何说明复用、增量与完成度？”时，先把岗位需要的能力拆成可验证要求，再用一到两个真实经历建立对应关系。表达顺序是结论、证据、个人贡献、边界与下一步；不要把团队成果全部归到个人，也不要用尚未测量的数据支撑结论。

**代码 / 场景：**

把一个月的提交按可演示里程碑重排：第一周建立文档与数据模型，第二周完成检索最小闭环，第三周接入 SSE 与交互，第四周补测试、说明和部署。为每个里程碑列出复用的成熟库、AI 辅助生成内容和自己亲自验证的部分，用 Git 记录证明速度来自范围控制而非虚构工作量。

**递进追问：**

1. **AI 编程工具在这一个月里具体替你做了什么？**
...
项目复盘应包含预期、实际偏差、根因、修复、验证和仍存限制。优先选择会影响正确性、恢复能力或用户体验的真实问题，并说明下次会把哪项机制和测试前置，而不是把“过于追求完美”包装成失败。
...
选择一个真实失败案例做五段复盘：预期是流式回答可恢复，实际是刷新后丢失未完成消息；用 requestId 和状态快照定位只保存了最终态；修复为持久化 pending 元数据并明确无法续传时的提示；最后补刷新、断网和重复提交测试。避免把“不足”回答成无关紧要的完美主义。

**递进追问：**

1. **这个失败为什么没有在开发阶段更早暴露？**
...
## Q8：生产项目缺少 React 经验时，应该如何补齐并说明边界？

**短回答：**

缺少生产 React 经验时应明确边界，再用可运行的小项目证明函数组件、Hooks、不可变更新、请求取消和测试能力。Vue 中的组件边界与状态机经验可以迁移，但依赖追踪、闭包和 Effect 生命周期必须重新学习。

**原理：**

回答“生产项目缺少 React 经验时，应该如何补齐并说明边界？”时，先把岗位需要的能力拆成可验证要求，再用一到两个真实经历建立对应关系。表达顺序是结论、证据、个人贡献、边界与下一步；不要把团队成果全部归到个人，也不要用尚未测量的数据支撑结论。

**代码 / 场景：**

准备一个最小 React 练习页复现真实 AI 交互：使用 reducer 管理消息状态，Effect 建立并清理流连接，AbortController 取消请求，列表 key 使用消息 ID。把它与 Vue 版本逐项对照，并保留测试记录；面试时直接说明生产经历以 Vue 为主，React 能力以该可运行样例为证据。

**递进追问：**

1. **Vue 经验能迁移到 React 的哪些部分，哪些不能直接照搬？**
...
- 不要把读过文档或做过 Demo 说成生产 React 经验，范围要明确。
...
## Q9：你是否接受学习 Go 并向全栈方向发展？

**短回答：**

学习 Go 的可信回答应区分可迁移基础与新增能力：HTTP、鉴权和数据访问经验可以复用，goroutine、channel、context、错误处理和服务治理需要通过代码、测试与排障继续验证，不能把“愿意学”等同于已经具备生产经验。

**原理：**

回答“你是否接受学习 Go 并向全栈方向发展？”时，先把岗位需要的能力拆成可验证要求，再用一到两个真实经历建立对应关系。表达顺序是结论、证据、个人贡献、边界与下一步；不要把团队成果全部归到个人，也不要用尚未测量的数据支撑结论。

**代码 / 场景：**

用 Go 写一个与 AI 知识工作台 无关的小接口验证学习能力：实现带 context 超时的并发聚合、结构化错误和单元测试，再用压测观察 goroutine 数与取消是否释放资源。将其与 Flask 同功能版本比较，说明已掌握的语法和运行模型，以及还未经历的生产部署与性能排障。

**递进追问：**

1. **为什么岗位需要 Go，而你过去主要用 Flask？**
...
- 不要仅以“语法简单”判断能快速掌握 Go，工程约定和并发取消更重要。
...
回答“如何安全、可信地使用 Codex、Cursor 等 AI 编程工具？”时，先把岗位需要的能力拆成可验证要求，再用一到两个真实经历建立对应关系。表达顺序是结论、证据、个人贡献、边界与下一步；不要把团队成果全部归到个人，也不要用尚未测量的数据支撑结论。

**代码 / 场景：**

挑选一段由 Codex 或 Cursor 协助完成的代码，保留原提示、候选补丁、人工修改和测试结果。现场说明先让工具列风险与测试，再限定文件范围生成方案，最后自己检查权限、错误处理和兼容性；同时准备一个曾拒绝 AI 建议的案例，证明决策没有外包。
...
## Q11：异地求职时，如何真实说明地点与到岗意愿？

**短回答：**

地点与到岗意愿属于真实约束，不是标准答案。应说明当前选择、可确认的时间范围和仍待决定的前置条件，并让招聘系统、简历和口头说法保持一致；公共题库不保存任何候选人的具体城市或搬迁承诺。

**原理：**

地点意愿的核心是信息一致性和可执行性。回答应把偏好、硬约束、确认节点与到岗条件分开：已经决定的部分给清楚结论，尚未决定的部分给确认时间，变化则说明新的信息和决策依据。这样既避免虚假承诺，也让招聘方能够评估实际安排。

**代码 / 场景：**

把地点回答写成三个事实字段：当前可接受范围、可确认的到岗时间、仍需处理的前置事项。然后检查招聘系统、简历和口头说法是否一致；如果尚未决定，就明确给出最终确认日期，不用模糊承诺换取流程机会。

**递进追问：**

1. **如果对方追问具体到岗时间，怎样回答才可信？**
...
## Q12：如何保证简历、README、演示与代码的完成度口径一致？

**短回答：**

简历、README、演示与代码应共享同一份完成度清单：已实现项要能定位代码和测试，实验项写明开关与限制，计划项写验收条件。时间线同样属于事实，不能用面试前临时补丁反向证明之前的描述准确。
...
逐条对照简历、GitHub README 和可运行代码制作完成度表：已实现写入口与测试，部分实现写限制，计划项写验收条件。对 RRF、MMR、向量迁移等尚未落地能力从“已实现”改成“设计或学习”，并用固定 commit 链接保存面试当天的真实版本。

**递进追问：**

1. **README 写了完整 RAG，代码里为什么只有关键词回退？**
...
- [MDN：客户端存储](https://developer.mozilla.org/en-US/docs/Learn_web_development/Extensions/Client-side_APIs/Client-side_storage)
- [MDN：CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/CORS)
- [OWASP：Improper Output Handling](https://genai.owasp.org/llmrisk/llm052025-improper-output-handling/)
