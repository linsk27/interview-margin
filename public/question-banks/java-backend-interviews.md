...
IoC 是把对象的创建与依赖管理交给容器；依赖注入是容器把所需对象传给 Bean 的实现方式。二者让业务代码面向接口协作，减少手工装配和硬编码依赖。
...
订单服务不要在类内直接 new 支付客户端，而是通过构造器接收 PaymentGateway。测试时注入假的实现，生产时由容器装配 HTTP 客户端；启动失败还能立即暴露缺失依赖。
...
为所有应用服务的方法统一记录耗时：切点匹配 service 包，环绕通知在 proceed 前后采样时间并打标签。若一个方法用 this 调另一个被增强方法，监控或事务可能不生效，应拆到另一个 Bean 或调整调用边界。
...
连接池包装 Bean 在初始化阶段校验地址与凭据，在销毁阶段关闭资源。不要在构造器里执行依赖其他 Bean 的远程请求，因为属性可能尚未完成注入；启动预热应放在明确的生命周期回调并设置超时。
...
创建订单与写本地事件表放在同一 public 服务方法中，使用同一个数据源事务提交。支付网关调用放到事务外或拆成状态机，避免远程超时让数据库连接和行锁长期占用。
...
批量导入方法内部调用本类的 saveOne，saveOne 上的 REQUIRES_NEW 没有生效。应把独立事务方法拆到另一个 Bean，再用集成测试故意制造异常，核对实际提交记录而不是只看注解。
...
JSON 创建订单请求先通过鉴权 Filter，再由参数解析器绑定 DTO 并做 Bean Validation；Controller 调用应用服务，统一异常处理器把业务异常映射成稳定错误码，消息转换器输出 JSON。
...
引入 JDBC starter 且配置 DataSource 属性后，类路径和缺失 Bean 条件满足，Boot 创建数据源相关 Bean；如果项目显式声明 DataSource，缺失 Bean 条件不再成立，用户配置接管。可通过条件评估报告定位为何未装配。
...
设计良好的构造器依赖不应形成环。底层容器可处理部分单例 Setter/字段循环依赖，但构造器环、prototype 环和某些代理场景仍会失败；现代 Spring Boot 默认也不允许循环引用。
...
OrderService 与 CouponService 互相注入通常说明职责或依赖方向错误。优先提取 PricingPolicy 或发布领域事件打断环，而不是开启允许循环依赖的配置；重构后用构造器注入让架构问题在启动时直接暴露。
...
PriceService 把 currentUserId 写进字段后再调用远程接口，高并发下会把 A 用户身份用于 B 请求。应把 userId 沿方法参数传递；计数指标使用专用线程安全指标组件，而不是在服务字段里做 ++。
...
一个 Boot Web 项目通常同时使用三者：容器装配 OrderService，MVC 把 POST /orders 映射到 Controller，Boot 自动配置服务器与 JSON 转换器。排查 404 看 MVC 映射，排查 Bean 缺失看容器与自动配置条件。
...
订单查询突然变慢时，先用 EXPLAIN ANALYZE 查看真实扫描行数和迭代器耗时，再检查锁等待与缓冲池命中；不要只看到 SQL 文本就直接添加索引，因为问题也可能是统计信息失真或连接池排队。
...
2. **查询缓存还在常规执行链中吗？**
   ...
   MySQL 8 已移除旧的全局查询缓存，回答时应区分版本，不能照搬早期版本执行流程。
...
订单、余额、库存必须依赖事务与故障恢复，使用 InnoDB。即便是只读归档表，也应先评估备份恢复、并发维护和统一运维成本，不因为“听说 MyISAM 查询快”就切换引擎。
...
订单按 user_id 和 created_at 查询最近记录，可建立 `(user_id, created_at)` 索引，让等值定位后顺序扫描时间范围。上线前用实际数据分布和 EXPLAIN 验证扫描行数，而不是为每个查询条件各建单列索引。
...
查询某用户最近订单的 `id,status,created_at`，索引 `(user_id, created_at, status)` 可能覆盖结果并减少回表。是否值得加入 status，要结合查询频率、字段长度和更新频率，再用执行计划确认 `using index` 与实际耗时。
...
查询 `tenant_id=? AND status=? AND created_at>=?`，优先按租户、等值筛选和常用范围设计联合索引。若还要按 created_at 排序，应一起评估；用不同参数分布执行 EXPLAIN ANALYZE，确认扫描范围。
...
不要用 `DATE(created_at)=?` 查询一天，可改成左闭右开时间范围；手机号 varchar 条件必须传字符串。优化前后比较 rows、filtered、key、Extra 和实际迭代器耗时，避免只看“是否出现 key”。
...
转账事务同时扣减 A、增加 B 并写流水，任一步失败都回滚；账户余额的非负约束还需条件更新或锁保护。测试应在提交前后模拟进程崩溃，并并发执行转账验证总额守恒。
...
2. **提交成功是否等于所有副本都已持久化？**
   ...
   取决于刷盘、复制与确认策略；本地提交、半同步或多数派确认对应不同故障边界。
...
报表需要事务内多次查询保持一致，可使用可重复读快照；扣库存必须用条件更新或锁定读，不能根据旧快照先判断再更新。并发测试要安排两个连接的精确执行顺序，记录每次读写结果。
...
长事务开启后一直不提交，会让旧版本无法及时回收，undo 空间增长并拖累查询。监控活跃事务与 history list，找到持有旧视图的会话，而不是只通过扩大磁盘掩盖问题。

**递进追问：**

1. **Read View 里通常要判断什么？**

   要结合当前事务、活跃事务 ID 集合及其上下界，判断某个版本在视图创建时是否已提交并可见。
...
两个转账事务都先按较小账户 ID 再按较大 ID 加锁，可减少反向等待；条件列必须有合适索引，避免扫描并锁住过大范围。捕获死锁错误后做有限退避重试，同时保存死锁日志定位语句。
...
误删数据可用全量备份加 binlog 回放到指定时间；实例异常重启主要依靠 redo 恢复已提交修改。恢复演练必须验证备份、binlog 保留和时间点，而不能把三种日志都称为“回滚日志”。
...
列表接口 p99 变慢，计划显示先扫描百万订单再排序。将过滤条件和排序组合进联合索引并改为游标分页后，比较扫描行数、p99 和写入成本；若只优化单个测试参数，还需用冷热用户数据复测。
...
商品详情读多写少，可把序列化结果按商品 ID 缓存并设置过期；库存最终事实仍落数据库。压测要关注 p99、命令复杂度和大键，而不是只看单机平均 QPS。
...
排行榜使用 Sorted Set，以积分为 score、用户 ID 为 member；用户标签交并集用 Set；访问 UV 可接受误差时用 HyperLogLog。设计时写明最大成员数、过期策略和是否需要精确结果。
...
会话缓存设置 TTL，并使用 allkeys-lfu 让高频会话更可能保留；上线前监控 expired_keys、evicted_keys、内存碎片和命中率。如果开始持续淘汰，应扩容或降低数据量，而非只拉长 TTL。
...
热门商品键到期时只允许一个请求异步重建，其余返回旧值或等待有限时间；批量预热的 TTL 加随机抖动。不存在商品 ID 使用参数边界和空值短缓存，所有回源都受限流器保护。
...
2. **逻辑过期有什么代价？**
   ...
   可能短时间返回旧数据，并需要独立的重建状态、失败重试和最大陈旧时间控制。
...
修改商品价格先在数据库事务提交，再发送包含商品 ID 和版本的失效事件；消费者幂等删除缓存。回填时比较版本，拒绝把旧版本覆盖新缓存，并监控失效消息积压。
...
会话缓存允许丢失且可重新登录，可把高可用和回源能力放在首位；若 Redis 保存难以重建的任务状态，则启用合适 AOF 策略并定期做恢复演练，记录恢复时间与最大可接受丢失量。
...
缓存容量单机足够但需要自动切换，可采用主从加 Sentinel；数据量和吞吐超过单机时再考虑 Cluster，并用 hash tag 让必须原子操作的相关键落同槽。演练主节点断网并记录切换时间和数据缺口。
...
2. **Cluster 为什么有 16384 个槽？**
   ...
   客户端和节点按槽路由与迁移，槽是键空间到节点之间的中间层，使重新分片无需逐键维护固定节点映射。
...
结算任务获得锁和递增版本后执行，数据库更新条件要求传入版本不小于当前值。故障测试暂停旧实例超过 TTL，让新实例接管，再恢复旧实例，确认旧写被拒绝且旧实例不会删新锁。
...
百万成员的活动用户 Set 应按时间或 hash 拆分，并用渐进式删除；爆款商品详情可用进程内短缓存和请求合并降低 Redis QPS。迁移前评估跨分片查询与一致性，不要只改键名。
...
下单成功后，邮件、积分和画像更新可消费 OrderCreated 事件异步完成；订单创建本身仍在主事务内返回明确结果。上线前测量峰值生产/消费速率、允许积压时间，并设计消费者失败后的重试与补偿。
...
支付结果事件使用可靠生产确认，Broker 配置足够副本；消费者在数据库事务内写业务结果和处理记录，提交后再确认消息。定期比对支付流水与订单状态，对缺失事件重新投递。
...
发券事件以 event_id 建唯一索引，在同一事务插入消费记录并创建优惠券。两个消费者并发时只有一个插入成功，另一个读取已完成结果并正常确认，不会发第二张券。
...
订单创建、支付、关闭事件都以 orderId 路由，消费者维护订单状态机。若先收到版本 3 而版本 2 缺失，暂存并告警；重复版本 2 通过幂等记录直接跳过，不把已关闭订单改回已支付。
...
订单 Topic 的总 lag 激增，分区指标显示一个大客户占满单分区。先隔离非核心事件并限制重试，再评估更细的路由键和增加分区；恢复过程中监控数据库连接池与 lag 下降斜率。
...
埋点与数据管道需要多次回放，可用 Kafka；订单状态事件若团队已有 RocketMQ 运维和事务消息经验，可沿用；轻量邮件路由可评估 RabbitMQ。选型文档写清峰值、保留期、顺序键和灾备。
...
初创订单系统先采用模块化单体，明确订单、库存、支付模块和接口；只有当发布节奏、团队边界或容量差异成为真实瓶颈时，再独立拆出服务，同时补齐追踪、超时和契约测试。
...
库存服务扩容后实例注册，订单服务收到变更并把请求分散到新节点。发布摘流时先标记不接新流量，再等待在途请求完成；调用失败按幂等性决定是否重试，并把实例与 traceId 写入日志。
...
创建订单使用 clientOrderNo 唯一约束。网关总预算 1 秒，订单服务调用库存单次 250ms，最多一次带抖动重试；若首次响应丢失，第二次根据唯一键返回原订单，不再次扣库存。
...
商品活动高峰按用户和接口令牌桶限流；推荐服务慢调用超过阈值后熔断，商品页降级为不显示推荐；订单提交使用独立连接池，确保推荐故障不会耗尽核心资源。
...
支付状态在主库不可确认时返回处理中，避免两个分区各自确认成功；商品详情在缓存与源站断连时可返回标注时间的旧版本。两条链路采用不同策略，并明确恢复后如何对账。
...
普通订单创建可在同一库写订单与 outbox，再由库存服务幂等消费实现最终一致；资金冻结若要求更强控制，可评估 TCC，并为 Confirm/Cancel 建唯一状态机和人工对账。
...
创建订单使用 POST，但客户端携带唯一 clientOrderNo，服务端用唯一约束实现业务幂等；重复请求返回原订单。库存版本冲突返回 409，限流返回 429 并携带合理重试提示，而不是所有错误都返回 200。
...
服务频繁短连接导致大量 TIME_WAIT，应先启用连接复用、检查客户端连接池和负载均衡超时，而不是随意缩短内核等待时间。抓包结合连接状态确认是哪一端主动关闭。
...
聊天网关用 Selector 维护大量长连接，只在连接可读时解析消息，再把耗时业务交给有界工作池。如果在事件循环中执行慢 SQL 或大文件计算，一个阻塞任务仍会拖慢该循环负责的全部连接。
...
先看流量、错误率和 p95/p99，再用 top/pidstat 定位进程与线程 CPU，vmstat/free 观察内存和换页，iostat 看磁盘，ss 看连接状态。应用侧用 jcmd、线程转储、GC 日志和 JFR 检查锁、热点栈、分配与暂停；数据库和 Redis 继续核对等待与延迟。每次只验证一个假设。

**代码 / 场景：**

发布后接口 p99 升高，top 显示 CPU 正常但线程池 active 已满；线程转储显示大量线程等待数据库连接，追踪又显示事务内远程调用。先回滚止损，再拆短事务并增加连接等待告警。
...
实例 CPU 100% 时连续 JFR 显示正则回溯占大头，先限流并回滚规则；若 RSS 增长但堆稳定，继续检查直接缓冲区、线程数量和 native memory，而不是盲目增大 Xmx。
...
导出任务 Service 是 singleton，但每次执行都需要新的 TaskContext。用 ObjectProvider<TaskContext>.getObject() 在任务开始时获取，而不是构造注入后长期复用同一个 prototype；请求作用域对象进入异步任务前则应提取必要值，不能继续依赖已结束的请求上下文。
...
同一事务连续两次按 id 查询用户且没有更新，一级缓存可能让第二次不访问数据库；若随后执行 updateUser，再查会重新访问。对“用户 + 角色”跨表视图，不要简单依赖 UserMapper 二级缓存，因为 RoleMapper 更新未必清理它；更稳妥的是关闭该缓存或使用有明确失效策略的集中缓存。
...
查询 50 个订单后逐个访问 order.items，若每个访问都触发 selectItemsByOrderId，就会产生 51 次 SQL。可以改成两次查询：先分页取 50 个订单，再用 WHERE order_id IN (...) 批量取明细并按 order_id 分组；这比把大明细表直接 JOIN 后再做错误的内存分页更可控。
...
用户修改昵称后，响应中记录写入时间或复制位点，接下来数秒的个人资料查询固定路由主库；商品推荐仍读副本。若订单确认页必须读到刚提交状态，可等待目标副本追到 GTID，超过几十毫秒立即回主库，并监控复制延迟与回退比例。
