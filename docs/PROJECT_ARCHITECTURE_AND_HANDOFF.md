...
- 14 个题库、741 道当前可见题已写入 SQLite。
- 原有简历与 JavaScript 181 题、320 道工程题、72 道 360 专项、40 道前端 × AI 题，
  以及全量重建后的 128 道 Java 高频题；结构化题解均带可核验来源。
...
- 当前回归基线：构建通过，741 题数据检查通过；42 个测试文件、225/225 项测试通过。
...
741 道当前可见题和小规模用户，但 `X-Expected-User-Id` 只防止跨账号错写，不解决同一账号多设备
...
- `server/content/community-banks/` 保存三套公开社区面经题库以及 Java 基础高频 50 题的数据；
...
- `public/content/diagrams/` 保存受控同源 SVG；Markdown 不允许远程图、Base64、
  空替代文本或原始内联 SVG。
- 660 道结构化富题解都通过正文长度、来源、重复模板和题号/标题一一对应门禁；题目 ID
  集合 SHA-256 固定测试可防止重排导致学习记录错绑。

因此：只迁移 Git 可以恢复程序和内置 741 题，但不能恢复账号、进度、批注和后台编辑结果。
...
- `db:check` 返回 14 个题库、741 道当前可见题、741 个唯一 ID；660 道结构化题解无缺段、无薄弱
...
部署后至少检查备用站首页不再跳回主站、`/catalog.json` 能返回 14 个题库/741 题，并模拟
...
Git 能保存前后端源代码、741 道当前内置题的 Markdown 和生成数据、数据库迁移、测试、
...
- 660 道结构化题已完成逐题 enrichment，后续仍应按月抽查版本敏感内容、网络协议、数据库
...
确认输出应为 14 个题库、741 题。主站动态目录不对时检查生产 SQLite；Vercel 静态目录不对时
