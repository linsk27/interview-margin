...
- 14 个题库、762 道当前可见题已写入 SQLite。
- 原有简历与 JavaScript 181 题、320 道工程题、72 道 360 专项、40 道前端 × AI 题，
  以及全量重建后的 149 道 Java 高频题（Java 基础 60 题、Java 后端 54 题、Java × AI 35 题）；结构化题解均带可核验来源。
...
- 当前回归基线：构建通过，762 题数据检查通过；42 个测试文件、225/225 项测试通过。
...
762 道当前可见题和小规模用户，但 `X-Expected-User-Id` 只防止跨账号错写，不解决同一账号多设备
...
5. 未登录的受邀者自行填写用户名、显示名称和 6–256 位密码；用户名为 2–64 位字母、数字、点、下划线或短横线。
...
- `server/content/community-banks/` 保存三套公开社区面经题库以及 Java 基础高频 60 题的数据；
...
- `public/content/diagrams/` 保存受控同源 SVG；Markdown 不允许远程图、Base64、
  空替代文本或原始内联 SVG。
- 681 道结构化富题解都通过正文长度、来源、重复模板和题号/标题一一对应门禁；题目 ID
  集合 SHA-256 固定测试可防止重排导致学习记录错绑。

因此：只迁移 Git 可以恢复程序和内置 762 题，但不能恢复账号、进度、批注和后台编辑结果。
...
Migration v4 为内置题换代增加两个题号接管触发器：新 seed 题复用同一题库题号时，旧 seed
题会先保留原 ID 并改为归档编号；旧代码回滚时也能反向接管题号。由此既不触发
`UNIQUE(bank_id, display_number)`，也不会删除旧题关联的进度、批注和来源记录。
...
- 42 个测试文件、225/225 项测试通过。
- `db:check` 返回 14 个题库、762 道当前可见题、762 个唯一 ID；681 道结构化题解无缺段、无薄弱
...
部署后至少检查备用站首页不再跳回主站、`/catalog.json` 能返回 14 个题库/762 题，并模拟
...
Git 能保存前后端源代码、762 道当前内置题的 Markdown 和生成数据、数据库迁移、测试、
...
- 681 道结构化题已完成逐题 enrichment，后续仍应按月抽查版本敏感内容、网络协议、数据库
...
约束：
- 非 Java 题不删除且 ID 不变；旧 Java 题在 v2 全量重建后归档，新题不继承旧进度。
...
确认输出应为 14 个题库、762 题。主站动态目录不对时检查生产 SQLite；Vercel 静态目录不对时
