...
### 面试边注是一个帮开发者准备技术面试的学习平台，提供前端、Java 和 AI 应用题库，以及白话题解、代码图解、AI 追问与评分、批注和复习计划。

游客打开就能刷题并体验 AI；登录后还能保存进度、收藏、批注和复习安排。
...
| 主动练习 | 先自己回答，再用固定规则获得 AI 面试官评分；它会分开说明做对了什么、为什么扣分、哪句表述需修正，然后再对照答案和安排 1 / 3 / 7 天复习 |
...
| 带可核验来源 | 762 题、1,949 条引用、1,166 个唯一 URL |
| 带术语翻译 | 492 题 |
| 带代码示例 | 576 题 |
| 带站内图解 | 53 题 |
| 自动化验证 | 最新回归与未通过项见 [2026-09-09验收记录](docs/ACCEPTANCE_2026-09-09.md) |
...
| AI | 浏览器只请求同源服务端代理；聊天兼容流式与一次性响应；评分按语义而非关键词判档，同一遗漏不重复扣分，并引用原句给出表述修正；模型只返回紧凑 JSON，默认最多 512 completion tokens，总分由服务端计算，并受独立的 5 次 / 10 分钟 / IP 子额度保护 |
| 部署 | 阿里云 ECS 单 Node 实例、systemd、Cloudflare Tunnel HTTPS；Vercel 保留游客快照，公开 AI 请求统一转发 ECS |
...
| [`server/app.js`](server/app.js) | `/api/ai-score` 权威题目读取、游客配额共享与评分入口 |
